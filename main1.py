...
def generate_clustered_data(true_k: int = 10) -> list[list[int]]:
    pts = []
    # Генерируем количество точек для каждого из true_k кластеров
    sizes = np.random.randint(low=30, high=100, size=true_k)

    for i in range(true_k):
        # Случайный центр для кластера в пределах экрана
        center_x = np.random.randint(50, WIDTH - 50)
        center_y = np.random.randint(50, HEIGHT - 50)
        
        # Случайный разброс (от плотного до рыхлого)
        spread = np.random.randint(10, 35)

        for _ in range(sizes[i]):
            x = np.random.normal(center_x, spread)
            y = np.random.normal(center_y, spread)
            
            # Ограничиваем координаты, чтобы точки не вылетали за экран
            x = max(0, min(WIDTH, x))
